...
const classifyTenantHost = (value = "") => {
  const classification = classifyDomainHost(value);

  if (classification.variant === PLATFORM_HOST_VARIANTS.LOCAL) {
    return HOST_KINDS.LOCAL_PLATFORM;
  }

  if (classification.variant === PLATFORM_HOST_VARIANTS.VERCEL) {
...
const getRequestHost = (req) => {
  ...
  return normalizeTenantHost(getRequestDomainHost(req) || "localhost");
};

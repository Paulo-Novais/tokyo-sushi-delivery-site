...
const normalizeDatabaseScope = (scope = {}) => {
  ...
  return Object.freeze({
    ...
    customerKey: normalizeSetting(scope.customerKey, 320),
    ...
    invitationTokenHash: normalizeSetting(scope.invitationTokenHash),
...
const buildScopedDatabaseUrl = (databaseUrl, scope = getDatabaseScope()) => {
  ...
  const options = [
    ...
    `-capp.customer_key=${normalizedScope.customerKey || "__none__"}`,
    ...
    `-capp.invitation_token_hash=${
      normalizedScope.invitationTokenHash || "__none__"
    }`,
...
const buildScopeQuery = (client, scope) => client`
  SELECT
    ...
    set_config('app.customer_key', ${scope.customerKey || "__none__"}, true),
    ...
    set_config(
      ...
      ${scope.invitationTokenHash || "__none__"},
      true
    ),

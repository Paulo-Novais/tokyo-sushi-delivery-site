    ...
    saveAdminUser(
      {
        user: {
          ...
          phone: adminUser.phone || adminUser.telefone || restaurantPayload.phone || restaurantPayload.whatsapp || "",
...
const handleAdminMaster = async (req, res) => {
  ...
  try {
    ...
    const metrics = await buildMasterMetricsSnapshot(usersPayload, platformSession.tenantContext);
    const snapshot = await getMasterPlatformSnapshot({
      metrics,
      usersPayload,
    });

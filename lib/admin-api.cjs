...
const getRestaurantAccessAddress = (restaurant = {}, domains = []) => {
  ...
  const managedDomain =
    matchingDomains.find(
      (domain) =>
        ...
        String(domain.primaryDomain || domain.domain || "").trim()
    ) || null;
  const sourceDomain = customDomain || managedDomain;
  const rawStatus = String(sourceDomain?.status || "").trim().toUpperCase();
  ...
  const restaurantSubdomain = String(restaurant.subdomain || "").trim();
  const restaurantDomain = String(restaurant.domain || "").trim();
  const safeRestaurantSubdomain =
    restaurantSubdomain && restaurantSubdomain !== customHost ? restaurantSubdomain : "";
  const safeRestaurantDomain =
    restaurantDomain && restaurantDomain !== customHost ? restaurantDomain : "";
  const managedHost = String(
    managedDomain?.primaryDomain ||
      managedDomain?.domain ||
      sourceDomain?.primaryDomain ||
      safeRestaurantSubdomain ||
      safeRestaurantDomain ||
      ""
  ).trim();
  const fallbackUrl = customDomain ? normalizeAccessUrl(managedHost) : "";
  const finalUrl = customDomain
    ? customIsActive
      ? normalizeAccessUrl(customHost)
      : fallbackUrl
    : normalizeAccessUrl(managedHost);

  if (!sourceDomain && !managedHost) {
    return {
      type: "NOT_CONFIGURED",
  ...
  return {
    type: customDomain ? "CUSTOM_DOMAIN" : "INOVAS_ADDRESS",
    typeLabel: customDomain ? "Dominio proprio" : "Endereco INOVAS",
    url: normalizeAccessUrl(customDomain ? customHost : managedHost),
    displayUrl: customDomain ? customHost : managedHost,
    finalUrl,
    fallbackUrl,
    status: customDomain ? customStatus : rawStatus || "ACTIVE",
    statusLabel: customDomain
      ...
      : sourceDomain?.statusLabel || (rawStatus === "ACTIVE" ? "Ativo" : rawStatus || "Nao configurado"),
    dnsStatus: customDomain?.dnsIntegrated === true ? "VERIFIED" : "PENDING",
    sslStatus:
      customDomain?.sslIntegrated === true
        ? "ACTIVE"
        : String(customDomain?.sslStatus || sourceDomain?.sslStatus || "PENDING").trim().toUpperCase(),
...
const buildInvitationDeliveryPayload = (req, invitationToken = "") => {
  const copyAllowed =
    process.env.NODE_ENV !== "production" ||
    String(process.env.INOVAS_ALLOW_INVITE_LINK_COPY || "").trim().toLowerCase() === "true";
  ...
  try {
      ...
      return json(res, 200, {
        ...
        invitation:
          result.user?.credentialMode === "INVITE"
            ? buildInvitationDeliveryPayload(req, result.invitationToken)
            : null,
      });
    }
    ...
    if (action === "resend-invite") {
      ...
      return json(res, 200, {
        ...
        user: result.user,
        message: result.message,
        invitation: buildInvitationDeliveryPayload(req, result.invitationToken),
      });
    }

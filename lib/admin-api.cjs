...
const {
  ...
  sendDiningOrder,
  updateDiningTabItem,
} = require("./cash-register-store.cjs");
const {
  getAdminAuditLog,
  getAdminFinance,
  getAdminMetrics,
...
const { getPlatformUrl } = require("./domain-topology.cjs");
const {
  guardSecurity,
  recordSecurityEvent,
...
const systemAuthHandler = createDomainAuthHandler(SESSION_AUDIENCES.SYSTEM);
const restaurantAuthHandler = createDomainAuthHandler(
  SESSION_AUDIENCES.RESTAURANT
);

const ADMIN_AUTH_ACTIONS = new Set(["login", "logout", "session", "accept-invite"]);
...
const PLATFORM_ONBOARDING_USER_TYPES = new Set(["MASTER", "SOCIO", "VENDEDOR"]);
const PLATFORM_SUBSCRIPTION_USER_TYPES = new Set(["MASTER", "SOCIO"]);
const PLATFORM_PANEL_USER_TYPES = new Set(["MASTER", "SOCIO"]);
...
const buildAdminPayload = (session, extra = {}) => {
  ...
  const isPlatformAdmin =
    session.platformScope === true || SYSTEM_USER_TYPES.includes(normalizedUserType);
  const restaurantKey = isPlatformAdmin ? "" : session.restaurantKey || "default";
  ...
  return {
    ...
    restaurantKey,
    userScope: isPlatformAdmin ? "SYSTEM" : "RESTAURANT",
    platformScope: isPlatformAdmin,
...
const buildAdminPayloadWithCommercialAccess = async (session, tenantContext = null) => {
  ...
  const isPlatformAdmin =
    session.platformScope === true || SYSTEM_USER_TYPES.includes(userType);
  ...
  const restaurantKey = session.restaurantKey || tenantContext?.restaurantKey || "default";
  const commercialAccess = await getRestaurantCommercialAccess(restaurantKey);
  const restaurantName =
    session.restaurantName ||
    session.restaurant ||
    (await resolveRestaurantNameByKey(restaurantKey));
...
const sendMasterOnboardingError = (res, error) => {
  const statusCode = Number(error?.statusCode || 500);
  const stableCode =
    statusCode >= 500
      ? "restaurant_onboarding_error"
      : error?.errorCode || "restaurant_onboarding_error";
  const message =
    statusCode >= 500
      ? "Não foi possível cadastrar o restaurante."
      : error?.publicMessage || error?.message || "Não foi possível cadastrar o restaurante.";
...
const hasPersistentAdminUserStore = () =>
  Boolean(String(process.env.DATABASE_URL || "").trim());

const runCapturedDomainHandler = async (handler, req) => {
  const captured = {
    statusCode: 200,
    headers: {},
    payload: null,
...
const handleAdminLogin = async (req, res) => {
  ...
  const hasConfiguredAccess =
    ...
    (hasAdminAuthConfig() || hasPersistentAdminUserStore());
  ...
  try {
    authenticatedAdmin = await runWithDatabaseScope(
      {
        audience: "authentication",
        login: identifier,
      },
      () => authenticateAdminUser(identifier, password, configuredUsers)
    );
  } catch (error) {
    recordSecurityFailure(req, {
      routeType: "admin-auth",
  ...
  const audience =
    String(authenticatedAdmin.userScope || "").trim().toUpperCase() === "SYSTEM"
  ...
  req.domainSecurityValidated = true;
  const captured = await runCapturedDomainHandler(
    audience === SESSION_AUDIENCES.SYSTEM
      ? systemAuthHandler
      : restaurantAuthHandler,
    req
  );

  if (captured.statusCode >= 400 || captured.payload?.success !== true) {
    return json(res, captured.statusCode, {
      error:
        captured.payload?.error?.message ||
        "Não foi possível criar a sessão de domínio.",
      errorCode:
        captured.payload?.error?.code || "domain_session_creation_failed",
    });
  }

  const domainSession = captured.payload.data.session;
  const tenantContext =
    audience === SESSION_AUDIENCES.RESTAURANT
      ? await getRequestTenantContext(req, {
          authenticatedSession: domainSession,
        })
      : null;
...
const handleAdminLogout = async (req, res) => {
  ...
  const systemLogout = await runCapturedDomainHandler(systemAuthHandler, req);
  const restaurantLogout = await runCapturedDomainHandler(
    restaurantAuthHandler,
    req
  );
  const cookies = [
    systemLogout.headers["Set-Cookie"],
    restaurantLogout.headers["Set-Cookie"],
    serializeDomainLogoutCookie(SESSION_AUDIENCES.SUPPORT, req),
    serializeAdminLogoutCookie(req),
  ].flat().filter(Boolean);
...
const handleAdminSession = async (req, res) => {
  ...
  try {
    ...
    try {
      domainSession = await requireSystemSession(req);
    } catch (error) {
      if (error?.errorCode !== "system_session_required") {
        throw error;
      }
    }
    if (!domainSession) {
      domainSession = await requireDirectRestaurantSession(req);
    }
    if (!domainSession) {
      return json(res, 200, {
        authenticated: false,
    ...
    const tenantContext =
      domainSession.audience === SESSION_AUDIENCES.RESTAURANT
        ? await getRequestTenantContext(req, {
            authenticatedSession: domainSession,
          })
        : null;
    const scopedSession = {
      ...
      audience: domainSession.audience,
      identityId: domainSession.identityId,
...
const handleAdminInvitationAcceptance = async (req, res) => {
  if (req.method !== "POST") {
    return sendMethodNotAllowed(res, "POST");
  }
  ...
  try {
    const payload = parseJsonBody(req.body, { strict: true });
    const result = await acceptAdminUserInvitation(payload, getConfiguredAdminUsers());
    ...
    return json(res, Number(error?.statusCode || 500), {
      ...
      error:
        Number(error?.statusCode || 500) >= 500
          ? "Nao foi possivel ativar este convite."
          : error?.message || "Convite invalido.",
      errorCode: error?.errorCode || "admin_invitation_error",
    });
  }
};
...
const handleAdminAuth = async (req, res) => {
  ...
  if (action === "accept-invite") {
    return handleAdminInvitationAcceptance(req, res);
  }
...
const resolveAdminRequiredPermissions = (req, group) => {
  ...
  if (group === "cash-register") {
    const permissionByAction = {
      snapshot: "cash_register_view",
      "configure-tables": "cash_register_configure",
      open: "cash_register_open",
      close: "cash_register_close",
      "open-tab": "cash_register_open_tab",
      "add-item": "cash_register_add_item",
      "update-item": "cash_register_add_item",
      "remove-item": "cash_register_add_item",
      "send-order": "cash_register_send_order",
      "begin-closing": "cash_register_close_tab",
      "reopen-tab": "cash_register_close_tab",
      "confirm-payment": "cash_register_confirm_payment",
    };
    return permissionByAction[action] || "cash_register_view";
  }
  ...
  if (group === "users") {
    if (["create", "create-context"].includes(action)) {
      return "users_create";
    }

    if (action === "save") {
      try {
        const payload = parseJsonBody(req.body, { strict: false });
        const userPayload =
          payload?.user && typeof payload.user === "object" ? payload.user : payload;
        return userPayload?.id ? "users_edit" : "users_create";
      } catch (error) {
    ...
    if (["status", "reset-password", "resend-invite"].includes(action)) {
      return "users_edit";
    }
...
const requireAdminAccessForRequest = async (req, requiredPermissions = []) => {
  ...
  domainSession = await requireRestaurantAccess(req);

  if (domainSession.audience === SESSION_AUDIENCES.SUPPORT) {
    throw buildHttpError(
      403,
      "A sessão de suporte deve usar as APIs /api/tenant explicitamente autorizadas.",
      "support_legacy_api_denied"
    );
  }
  ...
  assertTenantContextMatchesSession(tenantContext, domainSession);
...
const requirePlatformAccessForRequest = async (req, { allowedUserTypes = ["MASTER"], errorCode = "platform_access_required" } = {}) => {
  ...
  if (!allowedUserTypes.includes(userType)) {
    throw buildHttpError(
      403,
      "Acesso exclusivo para administradores autorizados da plataforma.",
      errorCode,
      {
        allowedUserTypes,
        userType,
...
const requirePlanAccessForRequest = async (req, group) => {
  const session = req.adminSession || requireAdminSession(req);
  const action =
    group === "dashboard"
...
const handleRestaurantSettingsList = async (req, res, session) => {
  const accessAddress = await getRestaurantAccessAddressForKey(
...
const handleRestaurantSettingsSave = async (req, res, session) => {
  ...
  return json(res, 200, {
    ...
    admin: buildAdminPayload(session),
    ...result,
    accessAddress: await getRestaurantAccessAddressForKey(
      session.restaurantKey
    ),
  });
};
...
const normalizeAdminUserScope = (value, userType = "") => {
  ...
  return SYSTEM_USER_TYPES.includes(userType) ? "SYSTEM" : "RESTAURANT";
};

const isSelectableRestaurantStatus = (value) =>
  ["ACTIVE", "ATIVO", "TRIAL", "PILOT", "CLIENTE_MODELO"].includes(
    String(value || "").trim().toUpperCase()
  );

const normalizeAccessUrl = (value) => {
  const candidate = String(value || "").trim().replace(/^https?:\/\//i, "").replace(/\/+$/, "");
  return candidate ? `https://${candidate}` : "";
};

const getRestaurantAccessAddress = (restaurant = {}, domains = []) => {
  const restaurantKey = normalizeAdminRestaurantKey(restaurant.restaurantKey || restaurant.key);
  const restaurantSlug = String(
    restaurant.slug || restaurant.restaurantKey || restaurant.key || ""
  ).trim();
  const managedPublicUrl = buildRestaurantPublicUrl(restaurantSlug);
  const managedPublicDisplayUrl =
    buildRestaurantPublicDisplayUrl(restaurantSlug);
  const matchingDomains = (Array.isArray(domains) ? domains : []).filter(
    (domain) =>
      normalizeAdminRestaurantKey(domain.restaurantKey) === restaurantKey &&
      domain.isSimulation !== true
  );
  const customDomain = matchingDomains.find((domain) => String(domain.customDomain || "").trim());

  if (!customDomain) {
    if (!managedPublicUrl) {
      return {
        type: "NOT_CONFIGURED",
        typeLabel: "Endereco nao configurado",
        url: "",
        displayUrl: "",
        finalUrl: "",
        fallbackUrl: "",
        ...
        dnsStatus: "NOT_AVAILABLE",
        sslStatus: "NOT_AVAILABLE",
        managementLabel: "",
      };
    }
    ...
    return {
      type: "INOVAS_ADDRESS",
      typeLabel: "Endereco INOVAS",
      url: managedPublicUrl,
      displayUrl: managedPublicDisplayUrl,
      finalUrl: managedPublicUrl,
      fallbackUrl: "",
      status: "ACTIVE",
      statusLabel: "Ativo",
      dnsStatus: "MANAGED",
      sslStatus: "MANAGED",
      managementLabel: "Gerenciado pelo INOVAS Food",
    };
  }

  const rawStatus = String(customDomain.status || "").trim().toUpperCase();
  const customIsActive = Boolean(
    customDomain &&
      rawStatus === "ACTIVE" &&
      customDomain.dnsIntegrated === true &&
      customDomain.sslIntegrated === true
  );
  const customStatus = customDomain
    ? customIsActive
      ? "ACTIVE"
      : ["SUSPENDED", "SUSPENSO"].includes(rawStatus)
        ? "SUSPENDED"
        : rawStatus === "ERROR"
          ? "CONFIGURATION_ERROR"
          : "PENDING_VERIFICATION"
    : "";
  const customHost = String(customDomain?.customDomain || customDomain?.domain || "").trim();
  const fallbackUrl = managedPublicUrl;
  const finalUrl = customIsActive
    ? normalizeAccessUrl(customHost)
    : fallbackUrl;

  return {
    type: "CUSTOM_DOMAIN",
    typeLabel: "Dominio proprio",
    url: normalizeAccessUrl(customHost),
    displayUrl: customHost,
    finalUrl,
    fallbackUrl,
    status: customStatus,
    statusLabel: customIsActive
      ? "Ativo"
      : customStatus === "SUSPENDED"
        ? "Suspenso"
        : customStatus === "CONFIGURATION_ERROR"
          ? "Erro de configuracao"
          : "Aguardando verificacao",
    dnsStatus: customDomain?.dnsIntegrated === true ? "VERIFIED" : "PENDING",
    sslStatus:
      customDomain?.sslIntegrated === true
        ? "ACTIVE"
        : String(customDomain?.sslStatus || "PENDING").trim().toUpperCase(),
    verifiedAt: customDomain?.verifiedAt || "",
    managementLabel: "Gerenciado pelo proprietario do dominio",
  };
};

const buildUserCreationContext = async (session, configuredUsers) => {
  const actor = buildAdminPayload(session);
  const usersPayload = await buildUsersPayload(configuredUsers);
  const snapshot = await getMasterPlatformSnapshot({ usersPayload });
  const actorType = normalizeAdminUserType(actor.userType || actor.tipo_usuario);
  const platformActor = actor.platformScope === true || SYSTEM_USER_TYPES.includes(actorType);
  const actorRestaurantKey = platformActor
    ? ""
    : normalizeAdminRestaurantKey(actor.restaurantKey);
  const restaurants = (Array.isArray(snapshot.restaurants) ? snapshot.restaurants : [])
    .filter(
      (restaurant) =>
        platformActor ||
        normalizeAdminRestaurantKey(restaurant.restaurantKey || restaurant.key) ===
          actorRestaurantKey
    )
    .map((restaurant) => ({
      key: normalizeAdminRestaurantKey(restaurant.restaurantKey || restaurant.key),
      tenantId: restaurant.tenantId || restaurant.tenant_id || "",
      restaurantId: restaurant.restaurantId || restaurant.restaurant_id || "",
      name: restaurant.name || restaurant.restaurantKey || restaurant.key,
      slug: restaurant.slug || "",
      status: String(restaurant.status || "").trim().toUpperCase(),
      statusLabel: restaurant.statusLabel || restaurant.status || "",
      plan: restaurant.plan || "",
      selectable: isSelectableRestaurantStatus(restaurant.status),
      accessAddress: getRestaurantAccessAddress(restaurant, snapshot.domains),
    }));
  const customProfile =
    (Array.isArray(usersPayload.profiles) ? usersPayload.profiles : []).find(
      (profile) => profile.type === "CUSTOM"
    ) || {
      type: "CUSTOM",
      label: "Personalizado",
      permissions: {},
  ...
  const restaurantProfiles = [
    ...(usersPayload.profileGroups?.restaurant || []),
    customProfile,
  ].filter(
    (profile) =>
      platformActor ||
      (profile.type !== "OWNER" && profile.type !== "MASTER")
  );

  return {
    actor: await buildAdminPayloadWithCommercialAccess(session, session.tenantContext),
    canSelectRestaurant: platformActor,
    fixedRestaurantKey: platformActor ? "" : actorRestaurantKey,
    restaurants,
    ...
    invitationDelivery: {
      ...
      linkCopyAllowed:
        ...
        String(process.env.INOVAS_ALLOW_INVITE_LINK_COPY || "").trim().toLowerCase() === "true",
      expiresInHours: Number(process.env.INOVAS_INVITE_TTL_HOURS || 48),
      requiredEnvironment: [
        "INOVAS_INVITE_EMAIL_PROVIDER",
        "INOVAS_INVITE_EMAIL_FROM",
        "INOVAS_INVITE_EMAIL_API_KEY",
      ],
    },
    userAssociation: {
      strategy: "global_identity_single_restaurant",
      multipleRestaurantsSupported: false,
      emailUniqueness: "global",
    },
  };
};

const buildInvitationDeliveryPayload = (
  req,
  invitationToken = "",
  accessAddress = {}
) => {
  const copyAllowed =
    process.env.NODE_ENV !== "production" ||
    String(process.env.INOVAS_ALLOW_INVITE_LINK_COPY || "").trim().toLowerCase() === "true";
  const configuredOrigin = String(process.env.INOVAS_ADMIN_PUBLIC_URL || "").trim();
  const origin = configuredOrigin || getPlatformUrl();
  const invitationUrl =
    copyAllowed && invitationToken && origin
      ? `${origin.replace(/\/+$/, "")}/admin/convite.html?token=${encodeURIComponent(invitationToken)}`
      : "";
  ...
  return {
    ...
    linkCopyAllowed: copyAllowed,
    invitationUrl,
    restaurantAccessUrl:
      accessAddress.finalUrl ||
      accessAddress.fallbackUrl ||
      accessAddress.url ||
      "",
  };
};

const getUserCreationAccessAddress = async (restaurantKey) =>
  getRestaurantAccessAddressForKey(restaurantKey);

const getRestaurantAccessAddressForKey = async (restaurantKey) => {
  const snapshot = await getMasterPlatformSnapshot();
  const restaurant = snapshot.restaurants.find(
    (entry) =>
      normalizeAdminRestaurantKey(entry.restaurantKey || entry.key) ===
      normalizeAdminRestaurantKey(restaurantKey)
  );

  return restaurant
    ? getRestaurantAccessAddress(restaurant, snapshot.domains)
    : {};
};
...
const scopeUsersPayloadForActor = (usersPayload = {}, actor = {}) => {
  ...
  if (actor.platformScope === true || SYSTEM_USER_TYPES.includes(actorType)) {
    return usersPayload;
  }
...
const assertUsersCommercialAccess = async (payload = {}, actor = {}, usersPayload = {}) => {
  ...
  const targetScope = normalizeAdminUserScope(
    userPayload.userScope || userPayload.user_scope || userPayload.scope,
    targetType
  );
  ...
  const restaurantKey =
    SYSTEM_USER_TYPES.includes(actorType)
...
const handleAdminUsers = async (req, res) => {
  ...
  try {
    ...
    if (action === "list") {
      ...
      return json(res, 200, {
        ok: true,
        admin: await buildAdminPayloadWithCommercialAccess(session, session.tenantContext),
    ...
    if (action === "create-context") {
      ...
      return json(res, 200, {
        ok: true,
        ...(await buildUserCreationContext(session, configuredUsers)),
      });
    }
    ...
    if (["create", "save"].includes(action)) {
      ...
      const result = await saveAdminUser(
        action === "create" ? { ...payload, forceCreate: true } : payload,
        actor,
        configuredUsers
      );
      const invitationAccessAddress =
        result.user?.credentialMode === "INVITE"
          ? await getUserCreationAccessAddress(result.user.restaurantKey)
          : {};
      ...
      return json(res, 200, {
        ...
        invitation:
          result.user?.credentialMode === "INVITE"
            ? buildInvitationDeliveryPayload(
                req,
                result.invitationToken,
                invitationAccessAddress
              )
            : null,
      });
    }
    ...
    if (action === "delete") {
      ...
      const payload = parseJsonBody(req.body, { strict: true });
      const result = await deleteAdminUser(payload, actor, configuredUsers);
    ...
    if (action === "resend-invite") {
      ...
      const payload = parseJsonBody(req.body, { strict: true });
      const result = await resendAdminUserInvitation(payload, actor, configuredUsers);
      const invitationAccessAddress = await getUserCreationAccessAddress(
        result.user.restaurantKey
      );
      ...
      return json(res, 200, {
        ...
        user: result.user,
        message: result.message,
        invitation: buildInvitationDeliveryPayload(
          req,
          result.invitationToken,
          invitationAccessAddress
        ),
    ...
    const fallbackMessages = {
      ...
      "create-context": "Nao foi possivel preparar a criacao do usuario.",
      create: "Nao foi possivel criar o usuario.",
      save: "Nao foi possivel salvar o usuario.",
      status: "Nao foi possivel alterar o status do usuario.",
      delete: "Nao foi possivel excluir o usuario.",
      "reset-password": "Nao foi possivel redefinir a senha do usuario.",
      "resend-invite": "Nao foi possivel gerar um novo convite.",
    };
    const fallbackErrorCodes = {
      list: "admin_users_error",
      "create-context": "admin_users_create_context_error",
      create: "admin_users_create_error",
      save: "admin_users_save_error",
      status: "admin_users_status_error",
      delete: "admin_users_delete_error",
      "reset-password": "admin_users_password_error",
      "resend-invite": "admin_users_invitation_error",
    };
...
const buildMasterMetricsSnapshot = async (usersPayload = {}) => {
  ...
  return {
    ...
    activeSystemUsers: systemUsers.filter((user) => user.status === "ACTIVE")
      .length,
    totalAccesses: systemUsers.filter(
      (user) => user.lastAccessAt || user.ultimo_acesso
    ).length,
    totalOrders: 0,
    totalCustomers: 0,
    totalReviews: 0,
    totalRevenue: 0,
    operationalMetricsExcluded: true,
  };
};

const assertCashRegisterPermission = (session, permission) => {
  if (session?.permissions?.[permission] !== true) {
    throw buildHttpError(
      403,
      "Seu perfil nao possui permissao para esta operacao do caixa.",
...
const handleAdminCashRegister = async (req, res) => {
  try {
    const session = requireAdminSession(req);
    const action = getAdminActionFromRequest(req) || "snapshot";
    const tenantOptions = getTenantStoreOptions(session);
    const actor = {
      ...buildAdminPayload(session),
    ...
    if (req.method === "GET") {
      if (action !== "snapshot" && action !== "cash-register") {
        return json(res, 404, {
          error: "Operacao do Caixa nao encontrada.",
          errorCode: "cash_register_action_not_found",
        });
      }
      const [snapshot, catalog] = await Promise.all([
        getCashRegisterSnapshot(tenantOptions),
        getAdminCatalog({}, tenantOptions),
      ]);
      return json(res, 200, {
        ok: true,
        admin: await buildAdminPayloadWithCommercialAccess(
          session,
    ...
    if (action === "configure-tables") {
      result = await configureDiningTables(body, actor, tenantOptions);
    } else if (action === "open") {
      result = await openCashRegister(body, actor, tenantOptions);
    } else if (action === "close") {
      result = await closeCashRegister(body, actor, tenantOptions);
    } else if (action === "open-tab") {
      result = await openDiningTab(body, actor, tenantOptions);
    } else if (action === "add-item") {
      result = await addDiningTabItem(body, actor, tenantOptions);
    } else if (action === "update-item") {
      result = await updateDiningTabItem(body, actor, tenantOptions);
    } else if (action === "remove-item") {
      result = await removeDiningTabItem(body, actor, tenantOptions);
    } else if (action === "send-order") {
      result = await sendDiningOrder(body, actor, tenantOptions);
    } else if (action === "begin-closing") {
      if (Number(body.discountAmount || 0) > 0) {
        assertCashRegisterPermission(session, "cash_register_discount");
      }
      if (body.serviceChargeEnabled === false) {
        assertCashRegisterPermission(
          session,
          "cash_register_remove_service"
        );
      }
      result = await beginDiningTabClosing(body, actor, tenantOptions);
    } else if (action === "reopen-tab") {
      result = await reopenDiningTab(body, actor, tenantOptions);
    } else if (action === "confirm-payment") {
      result = await confirmDiningPayment(body, actor, tenantOptions);
    } else {
      return json(res, 404, {
        error: "Operacao do Caixa nao encontrada.",
        errorCode: "cash_register_action_not_found",
    ...
    const snapshot = await getCashRegisterSnapshot(tenantOptions);
    return json(res, 200, {
      ok: true,
      admin: await buildAdminPayloadWithCommercialAccess(
        session,
    ...
    return json(res, Number(error?.statusCode || 500), {
      error: error?.message || "Nao foi possivel concluir a operacao do Caixa.",
      errorCode:
        error?.errorCode ||
        (error?.statusCode ? "cash_register_operation_error" : "internal_error"),
      requiredPermissions: error?.requiredPermissions || undefined,
      expectedTotal: error?.expectedTotal,
      paymentTotal: error?.paymentTotal,
      openTabs: error?.openTabs,
      pendingItems: error?.pendingItems,
    });
  }
};
...
const handleMasterOnboardRestaurant = async (req, res, session) => {
  ...
  const appearance = restaurantPayload.appearance && typeof restaurantPayload.appearance === "object"
    ? restaurantPayload.appearance
    : {};
  const restaurantSettingsPayload = {
    restaurantName: onboarding.restaurant.name,
    logoUrl: restaurantPayload.logoUrl || appearance.logoUrl || "",
    bannerUrl: restaurantPayload.bannerUrl || appearance.bannerUrl || "",
    primaryColor: restaurantPayload.primaryColor || appearance.primaryColor || "",
    secondaryColor: restaurantPayload.secondaryColor || appearance.secondaryColor || "",
  ...
  const [restaurantSettingsResult, deliverySettingsResult, adminUserResult] =
    await runWithDatabaseScope(
      ...
      () =>
        Promise.all([
          updateRestaurantSettings(restaurantSettingsPayload, actor, {
            tenantContext,
          }),
          updateDeliverySettings(
            buildDeliverySettingsFromOnboarding(restaurantPayload),
            actor,
            { tenantContext }
          ),
          saveAdminUser(
            {
              user: {
                ...
                name:
                  ...
                  `Admin ${onboarding.restaurant.name}`,
  ...
  try {
    if (req.method === "POST") {
      if (["onboard-restaurant", "onboarding", "restaurants"].includes(action)) {
        const platformSession = await requirePlatformAccessForRequest(req, {
          allowedUserTypes: [...PLATFORM_ONBOARDING_USER_TYPES],
          errorCode: "platform_onboarding_access_required",
        });
        return await handleMasterOnboardRestaurant(req, res, platformSession);
      }

      if (["subscription", "subscriptions"].includes(action)) {
        const platformSession = await requirePlatformAccessForRequest(req, {
          allowedUserTypes: [...PLATFORM_SUBSCRIPTION_USER_TYPES],
          errorCode: "platform_subscription_access_required",
        });
        return await handleMasterSubscriptionUpdate(req, res, platformSession);
      }
    ...
    const platformSession = await requirePlatformAccessForRequest(req, {
      allowedUserTypes: [...PLATFORM_PANEL_USER_TYPES],
      errorCode: "master_access_required",
    });

    const usersPayload = await buildUsersPayload(getConfiguredAdminUsers());
    const metrics = await buildMasterMetricsSnapshot(usersPayload);
    const snapshot = await getMasterPlatformSnapshot({
      metrics,
      usersPayload,
    });
    ...
    return json(res, 200, {
      ...snapshot,
      admin: buildAdminPayload(platformSession),
      activeModule: action === "master" ? "overview" : action,
    });
  } catch (error) {
    if (req.method === "POST" && ["onboard-restaurant", "onboarding", "restaurants"].includes(action)) {
      return sendMasterOnboardingError(res, error);
    }
...
const handleAdminApi = async (req, res) => {
  ...
  try {
    ...
    if (group === "master" && req.method === "POST" && ["onboard-restaurant", "onboarding", "restaurants"].includes(action)) {
      return sendMasterOnboardingError(res, error);
    }
  ...
  if (group === "cash-register") {
    return handleAdminCashRegister(req, res);
  }
...
module.exports.getRestaurantAccessAddress = getRestaurantAccessAddress;

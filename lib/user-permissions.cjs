...
const saveAdminUser = async (payload = {}, actor = {}, configuredUsers = []) => {
  ...
  if (userScope === "RESTAURANT" && RESTAURANT_USER_TYPE_SET.has(userType) && !isValidPhone(phone)) {
  ...
  const nextUser = normalizeStoredUser({
    ...
    tenantId: userScope === "SYSTEM" ? "" : userPayload.tenantId || userPayload.tenant_id || buildTenantId(restaurantKey),
    restaurantId: userScope === "SYSTEM" ? "" : userPayload.restaurantId || userPayload.restaurant_id || buildRestaurantId(restaurantKey),
    restaurantKey,
    name,
    login,
    email,

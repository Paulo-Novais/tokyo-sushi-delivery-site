...
const USER_TYPE_LABELS = Object.freeze({
  ...
  VENDEDOR: "Vendedor",
  COMERCIAL: "Comercial",
  FINANCEIRO_INOVAS: "Financeiro INOVAS",
...
const USER_STATUS_LABELS = Object.freeze({
  ...
  PASSWORD_RESET_REQUIRED: "Redefinição de senha obrigatória",
});
...
const SYSTEM_USER_MANAGEABLE_TYPES = Object.freeze({
  ...
  DESENVOLVEDOR: Object.freeze(["SUPORTE", "VENDEDOR", "AUDITOR"]),
  SUPORTE: Object.freeze(["AUDITOR"]),
...
const CASH_REGISTER_PERMISSION_DEFINITIONS = Object.freeze([
  { key: "cash_register_view", label: "Visualizar salao" },
  { key: "cash_register_configure", label: "Configurar mesas" },
  { key: "cash_register_open", label: "Abrir caixa" },
  { key: "cash_register_close", label: "Fechar caixa" },
  { key: "cash_register_open_tab", label: "Abrir comanda" },
  { key: "cash_register_add_item", label: "Adicionar e editar itens" },
  { key: "cash_register_send_order", label: "Enviar pedido" },
  { key: "cash_register_discount", label: "Aplicar desconto" },
  { key: "cash_register_remove_service", label: "Remover taxa de servico" },
  { key: "cash_register_close_tab", label: "Fechar conta" },
  { key: "cash_register_confirm_payment", label: "Confirmar pagamento" },
  { key: "cash_register_history_view", label: "Visualizar historico" },
]);
const CASH_REGISTER_PERMISSION_KEYS = Object.freeze(
  CASH_REGISTER_PERMISSION_DEFINITIONS.map(({ key }) => key)
);
...
const normalizeStatus = (value, fallback = "ACTIVE") => {
  ...
  if (["BLOQUEADO", "BLOCKED"].includes(normalizedValue)) {
    return "BLOCKED";
  }

  if (["INATIVO", "INACTIVE", "DISABLED"].includes(normalizedValue)) {
    return "INACTIVE";
  }

  if (
    ["PENDENTE", "PENDING", "PENDING_INVITE", "INVITED", "CONVIDADO"].includes(
      normalizedValue
    )
  ) {
    return "PENDING";
...
const PROFILE_DEFAULT_PERMISSIONS = Object.freeze({
  ...
  CAIXA: createPermissions([
    ...
    "cash_register_open",
    "cash_register_close",
    "cash_register_open_tab",
    "cash_register_add_item",
    "cash_register_send_order",
    "cash_register_remove_service",
    "cash_register_close_tab",
    "cash_register_confirm_payment",
    "cash_register_history_view",
  ]),
...
const getDefaultPermissionsForType = (userType) => {
  if (SYSTEM_USER_TYPE_SET.has(userType)) {
...
const assertPermissionDependencies = (permissions = {}) => {
  ...
  const selectedCashRegisterPermission = CASH_REGISTER_PERMISSION_KEYS.find(
    (permission) =>
      permission !== "cash_register_view" &&
      normalizedPermissions[permission] === true
  );
  if (
    selectedCashRegisterPermission &&
    normalizedPermissions.cash_register_view !== true
  ) {
    throw buildHttpError(
      400,
      `A permissao ${selectedCashRegisterPermission} exige cash_register_view.`,
      "permission_dependency_missing",
      {
        permission: selectedCashRegisterPermission,
        requiredPermission: "cash_register_view",
      }
    );
  }
  ...
  schemaReadyPromise = (async () => {
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS admin_users (
        ...
        grant_overrides_json JSONB NOT NULL DEFAULT '[]'::jsonb,
        deny_overrides_json JSONB NOT NULL DEFAULT '[]'::jsonb,
...
const normalizeStoredUser = (user = {}) => {
  ...
  return {
    ...
    department: normalizeText(
      user.department || user.department_name || user.sector || user.setor,
      120
    ),
    internalNote: normalizeText(
      user.internalNote || user.internal_note || user.notes || user.observacao,
      500
    ),
    ...
    baseRole: normalizeText(user.baseRole || user.base_role, 80).toUpperCase(),
    grantOverrides: Array.isArray(
      user.grantOverrides || user.grant_overrides_json
    )
      ? cloneJson(user.grantOverrides || user.grant_overrides_json).slice(0, 200)
      : [],
    denyOverrides: Array.isArray(
      user.denyOverrides || user.deny_overrides_json
    )
      ? cloneJson(user.denyOverrides || user.deny_overrides_json).slice(0, 200)
      : [],
    profileVersion: normalizeText(
      user.profileVersion || user.profile_version,
      80
    ),
...
const readUsersFromNeon = async () => {
  ...
  return rows.map((row) =>
    normalizeStoredUser({
      ...
      grantOverrides: row.grant_overrides_json,
      denyOverrides: row.deny_overrides_json,
...
const writeUsersToNeon = async (users) => {
  ...
  const databaseScope = getDatabaseScope();

  if (databaseScope.audience === "authentication") {
    if (
      normalizedUsers.length !== 1 ||
      !databaseScope.login ||
      !databaseScope.invitationTokenHash
    ) {
      throw buildHttpError(
        403,
        "Escopo de aceite de convite invalido.",
        "invalid_invitation_database_scope"
      );
    }

    const [user] = normalizedUsers;
    const updated = await sql`
      ...
      SET
        ...
        invitation_token_hash = ${user.invitationTokenHash},
        invitation_expires_at = ${user.invitationExpiresAt || null},
        invitation_created_at = ${user.invitationCreatedAt || null},
        invitation_sent_at = ${user.invitationSentAt || null},
        invitation_used_at = ${user.invitationUsedAt || null},
        audit_json = ${JSON.stringify(user.auditTrail)}::jsonb,
        updated_at = ${user.updatedAt}
      WHERE id = ${user.id}
        AND LOWER(login) = LOWER(${databaseScope.login})
        AND invitation_token_hash = ${databaseScope.invitationTokenHash}
      RETURNING id
    `;
  ...
  for (const user of normalizedUsers) {
    try {
      ...
      VALUES (
        ...
        ${user.baseRole},
        ${JSON.stringify(user.grantOverrides)}::jsonb,
        ${JSON.stringify(user.denyOverrides)}::jsonb,
        ${user.profileVersion},
        ${user.credentialMode},
      ...
      DO UPDATE SET
        ...
        grant_overrides_json = EXCLUDED.grant_overrides_json,
        deny_overrides_json = EXCLUDED.deny_overrides_json,
      ...
      console.error("[admin-users-store]", {
        audience: scope.audience,
        errorCode: error?.code || "internal_error",
        message: error?.message || "Unexpected admin user persistence error.",
        restaurantId: user.restaurantId,
        restaurantKey: user.restaurantKey,
        supportMode: scope.supportMode,
...
const ensureConfiguredAdminUsers = async (configuredUsers = []) => {
  const storageMode = getStorageMode();
  const databaseScope =
    storageMode === "neon" ? getDatabaseScope() : { audience: "file" };
  let normalizedConfiguredUsers = configuredUsers
    .map(normalizeConfiguredAdminUser)
    .filter(Boolean);

  if (
    !normalizedConfiguredUsers.length ||
    storageMode === "disabled" ||
    databaseScope.audience === "authentication"
  ) {
    return;
  }

  if (databaseScope.audience === "system") {
    normalizedConfiguredUsers = normalizedConfiguredUsers.filter(
      (user) => user.userScope === "SYSTEM"
    );
  } else if (
    ["restaurant", "support", "provisioning"].includes(
      databaseScope.audience
    )
  ) {
    normalizedConfiguredUsers = normalizedConfiguredUsers.filter(
      (user) =>
        user.userScope === "RESTAURANT" &&
        user.tenantId === databaseScope.tenantId &&
        user.restaurantId === databaseScope.restaurantId
    );
  }

  if (!normalizedConfiguredUsers.length) {
    return;
  }
...
const readAllUsers = async (configuredUsers = []) => {
  const databaseScope =
    getStorageMode() === "neon" ? getDatabaseScope() : { audience: "file" };
  if (databaseScope.audience !== "authentication") {
...
const serializePermissionModules = () => [
  ...
  })),
  {
    key: "cash_register",
    label: "Caixa e Salao",
    permissions: CASH_REGISTER_PERMISSION_DEFINITIONS.map((permission) => ({
      key: permission.key.replace(/^cash_register_/, ""),
      action: permission.key.replace(/^cash_register_/, ""),
...
const serializeUser = (user) => {
  ...
  return {
    ...
    internalNote: normalizedUser.internalNote,
    ...
    baseRole: normalizedUser.baseRole || normalizedUser.userType,
    grantOverrides: cloneJson(normalizedUser.grantOverrides),
    denyOverrides: cloneJson(normalizedUser.denyOverrides),
    profileVersion: normalizedUser.profileVersion,
...
const serializeAccessUser = (user) => {
  ...
  return {
    ...
    baseRole: serializedUser.baseRole,
    grantOverrides: serializedUser.grantOverrides,
    denyOverrides: serializedUser.denyOverrides,
    profileVersion: serializedUser.profileVersion,
...
const recordAdminUserAccess = async (login, configuredUsers = []) => {
  ...
  if (storageMode === "neon") {
    ...
    const rows = await getSql()`
      ...
      RETURNING *
    `;
    return rows[0] ? serializeUser(normalizeStoredUser(rows[0])) : null;
  }
...
const assertScopedMasterInvariant = (users) => {
  const databaseScope =
    getStorageMode() === "neon" ? getDatabaseScope() : { audience: "file" };
  ...
  assertSingleActiveMaster(users);
};

const assertActiveOwnerRemains = (users, restaurantKey) => {
  const normalizedRestaurantKey = normalizeRestaurantKey(
    restaurantKey,
    RESTAURANT_KEY
  );
  const activeOwners = users.filter(
    (user) =>
      normalizeUserScope(
        user.userScope,
        normalizeUserType(user.userType, "CUSTOM"),
        user.platformScope === true
      ) === "RESTAURANT" &&
      normalizeRestaurantKey(user.restaurantKey, RESTAURANT_KEY) ===
        normalizedRestaurantKey &&
      normalizeUserType(user.userType, "CUSTOM") === "OWNER" &&
      normalizeStatus(user.status, "BLOCKED") === "ACTIVE"
  );

  if (!activeOwners.length) {
    throw buildHttpError(
      409,
      "O restaurante deve manter ao menos um Owner ativo.",
...
const saveAdminUser = async (payload = {}, actor = {}, configuredUsers = []) => {
  ...
  const internalNote = normalizeText(
    userPayload.internalNote ||
      userPayload.internal_note ||
      userPayload.notes ||
      existingUser?.internalNote,
    500
  );
  ...
  const nextUser = normalizeStoredUser({
    ...
    baseRole: normalizeText(
      userPayload.baseRole || userPayload.base_role || userType,
      80
    ).toUpperCase(),
    grantOverrides: Array.isArray(userPayload.grantOverrides)
      ? userPayload.grantOverrides
      : existingUser?.grantOverrides || [],
    denyOverrides: Array.isArray(userPayload.denyOverrides)
      ? userPayload.denyOverrides
      : existingUser?.denyOverrides || [],
    profileVersion: normalizeText(
      userPayload.profileVersion ||
        userPayload.profile_version ||
        existingUser?.profileVersion ||
        "legacy-v1",
      80
    ),
    credentialMode,
  ...
  assertScopedMasterInvariant(nextUsers);
  if (
    existingUser &&
    normalizeUserScope(
      existingUser.userScope,
    ...
    normalizeUserType(existingUser.userType, "CUSTOM") === "OWNER"
  ) {
    assertActiveOwnerRemains(nextUsers, existingUser.restaurantKey);
  }
  try {
    await writeStoredUsers(nextUsers);
  } catch (error) {
...
const deleteAdminUser = async (payload = {}, actor = {}, configuredUsers = []) => {
  ...
  if (
    normalizeUserScope(
      ...
      targetUser.userType,
      targetUser.platformScope === true
    ) === "RESTAURANT" &&
    normalizeUserType(targetUser.userType, "CUSTOM") === "OWNER"
  ) {
    assertActiveOwnerRemains(nextUsers, targetUser.restaurantKey);
  }
  await writeStoredUsers(nextUsers);
...
const resendAdminUserInvitation = async (payload = {}, actor = {}, configuredUsers = []) => {
  ...
  const now = new Date().toISOString();
  const latestReissue = [...(targetUser.auditTrail || [])]
    .reverse()
    .find((event) => event?.action === "INVITATION_REISSUED");
  if (
    latestReissue?.createdAt &&
    Date.now() - new Date(latestReissue.createdAt).getTime() < 30_000
  ) {
    throw buildHttpError(
      429,
      "Aguarde antes de reenviar um novo convite.",
...
module.exports = {
  ALL_PERMISSION_KEYS,
  CASH_REGISTER_PERMISSION_DEFINITIONS,
  CASH_REGISTER_PERMISSION_KEYS,
  DEVELOPER_PERMISSION_KEYS,
  MODULE_PERMISSION_KEYS,
  PERMISSION_ACTIONS,

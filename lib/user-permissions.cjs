...
const FULL_RESTAURANT_USER_TYPES = new Set(["MASTER", "DESENVOLVEDOR", "OWNER", "GERENTE"]);
...
const getEffectivePermissions = (user) => {
  ...
  if (FULL_RESTAURANT_USER_TYPES.has(userType) || PROFILE_DEFAULT_PERMISSIONS[userType]) {
...
const assertCanManageUser = ({ actor = {}, targetUser = {}, existingUser = null, requestedRestaurantKey = RESTAURANT_KEY }) => {
  ...
  if (targetType === "MASTER" || targetType === "DESENVOLVEDOR" || targetType === "OWNER") {
    throw buildHttpError(
      403,
      "OWNER pode criar apenas usuarios internos do proprio restaurante.",
      "owner_cannot_manage_platform_user"
    );
  }

  if (!RESTAURANT_INTERNAL_USER_TYPES.has(targetType)) {
...
const saveAdminUser = async (payload = {}, actor = {}, configuredUsers = []) => {
  ...
  if (V1_1_USER_TYPES.includes(userType) && userType !== "MASTER" && !isValidPhone(phone)) {
    throw buildHttpError(400, "Informe um telefone valido para o usuario.", "invalid_user_phone");
  }

  if (V1_1_USER_TYPES.includes(userType) && userType !== "MASTER" && !restaurantKey) {

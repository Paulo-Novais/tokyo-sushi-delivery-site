...
const USER_TYPES = Object.freeze(["MASTER", "DESENVOLVEDOR", "OWNER", "CUSTOM"]);
const USER_STATUSES = Object.freeze(["ACTIVE", "BLOCKED"]);
...
const FULL_RESTAURANT_USER_TYPES = new Set(["MASTER", "DESENVOLVEDOR", "OWNER"]);
...
const normalizeUserType = (value, fallback = "CUSTOM") => {
  ...
  return USER_TYPES.includes(normalizedValue) ? normalizedValue : fallback;
};
...
const createFullRestaurantPermissions = ({ includeDeveloperPermissions = false } = {}) => {
  const permissions = MODULE_PERMISSION_KEYS.reduce((result, key) => {
    result[key] = true;
    return result;
  }, {});
...
const getEffectivePermissions = (user) => {
  ...
  if (FULL_RESTAURANT_USER_TYPES.has(userType)) {
...
const normalizeStoredUser = (user = {}) => {
  ...
  const userType = normalizeUserType(user.userType || user.tipo_usuario, "CUSTOM");
  const restaurantKey = normalizeRestaurantKey(user.restaurantKey || user.restaurant_key, RESTAURANT_KEY);
  ...
  return {
    ...
    tenantId: normalizeRestaurantKey(user.tenantId || user.tenant_id, buildTenantId(restaurantKey)),
    restaurantId: normalizeRestaurantKey(user.restaurantId || user.restaurant_id, buildRestaurantId(restaurantKey)),
    restaurantKey,
    name,
    login,
...
const normalizeConfiguredAdminUser = (user = {}) => {
  ...
  return {
    ...
    tenantId: buildTenantId(RESTAURANT_KEY),
    restaurantId: buildRestaurantId(RESTAURANT_KEY),
    restaurantKey: RESTAURANT_KEY,
    passwordHash,
    password,
  };
...
const authenticateAdminUser = async (identifier, password, configuredUsers = []) => {
  ...
  if (user.status !== "ACTIVE") {
    throw buildHttpError(403, "Usuario bloqueado.", "admin_user_blocked");
  }
...
const assertAtLeastOneActiveMaster = (users) => {
  const activeMasters = users.filter(
    (user) => user.status === "ACTIVE" && normalizeUserType(user.userType) === "MASTER"
  );
...
const saveAdminUser = async (payload = {}, actor = {}, configuredUsers = []) => {
  ...
  const login = normalizeIdentifier(userPayload.login);
  ...
  const status = normalizeStatus(userPayload.status, existingUser?.status || "ACTIVE");
  ...
  const requestedRestaurantKey = normalizeRestaurantKey(
  ...
  if (users.some((user) => user.login === login && user.id !== existingUser?.id)) {
    throw buildHttpError(409, "Ja existe um usuario com este login.", "duplicate_user_login");
  }
  ...
  const nextUser = normalizeStoredUser({
    ...
    tenantId: userPayload.tenantId || userPayload.tenant_id || buildTenantId(restaurantKey),
    restaurantId: userPayload.restaurantId || userPayload.restaurant_id || buildRestaurantId(restaurantKey),
    restaurantKey,
    name,
    login,
    email,
    ...
    status,
    userType,
    permissions: userType === "CUSTOM" ? normalizePermissions(userPayload.permissions) : {},
  ...
  assertAtLeastOneActiveMaster(nextUsers);
  await writeStoredUsers(nextUsers);
...
const setAdminUserStatus = async (payload = {}, actor = {}, configuredUsers = []) => {
  ...
  return {
    ...
    users: nextUsers.map(serializeUser),
    message: nextStatus === "ACTIVE" ? "Usuario desbloqueado com sucesso." : "Usuario bloqueado com sucesso.",
  };
};

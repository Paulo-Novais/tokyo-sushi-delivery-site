...
const getTenantMode = () =>
  require("./tenant-resolution.cjs").getTenantMode();
...
const buildPublicRouteProjection = (state) =>
  normalizeArray(state.restaurants)
    .map((restaurant) => {
      ...
      const domainHost = domain && isResolvableDomain(domain)
        ? normalizeDomainHost(
...
const buildPublicRouteProjectionQueries = (sql, state) => {
  const routes = buildPublicRouteProjection(state);
  const routeQueries = routes.map((route) => sql`
...
const toPublicRouteRecord = (restaurant = {}, domain = null) => {
  const restaurantKey = normalizeSlug(
    restaurant.restaurantKey || restaurant.key || restaurant.slug,
    "",
    120
  ...
  return {
    ...
    slug,
    restaurant_name: normalizeText(restaurant.name, restaurantKey, 160),
    domain_host: domain
      ? normalizeDomainHost(
          domain.customDomain || domain.primaryDomain || domain.domain,
          ""
      ...
      : "",
    status: normalizeText(restaurant.status, "ACTIVE", 80).toUpperCase(),
    public_url: buildRestaurantPublicUrl(slug),
    dns_integrated: domain?.dnsIntegrated === true,
    ssl_integrated: domain?.sslIntegrated === true,
    domain_status: normalizeText(domain?.status, "", 80).toUpperCase(),
    domain_is_simulation: domain?.isSimulation === true,
  };
};

// Repository boundary only. Resolution policy belongs to tenant-resolution.cjs.
// Every Neon lookup enters the public RLS audience before touching the route
// projection, regardless of the caller's current AsyncLocalStorage scope.
const findPublicRestaurantRoute = async ({
  slug = "",
  host = "",
  restaurantKey = "",
} = {}) => {
  if (getStorageMode() === "neon") {
    return runWithDatabaseScope(
      { audience: "public" },
      () => readPublicRoute({ slug, host, restaurantKey })
    );
  }
  ...
  if (normalizedRestaurantKey) {
    restaurant = state.restaurants.find(
      (entry) =>
        entry.restaurantKey === normalizedRestaurantKey ||
        entry.key === normalizedRestaurantKey
    ) || null;
  } else if (normalizedSlug) {
    restaurant = state.restaurants.find(
      (entry) => normalizeRestaurantSlug(entry.slug) === normalizedSlug
    ) || null;
  } else if (normalizedHost) {
    domain = state.domains.find(
      (entry) => getDomainCandidates(entry).includes(normalizedHost)
    ) || null;
    restaurant = domain
      ? state.restaurants.find(
          (entry) =>
            entry.restaurantKey === domain.restaurantKey ||
            entry.key === domain.restaurantKey
        ) || null
      : null;
  }
  ...
  domain = domain || state.domains.find(
    (entry) =>
      entry.restaurantKey === restaurant.restaurantKey &&
      entry.isSimulation !== true
  ) || null;

  return toPublicRouteRecord(restaurant, domain);
};
...
const writeNeonStore = async (state) => {
  ...
  await sql.transaction([
    stateQuery,
    ...buildPublicRouteProjectionQueries(sql, normalizedState),
  ]);
};
...
const resolveRestaurantByHost = (host = "") =>
  require("./tenant-resolution.cjs").resolveRestaurantByHost(host);

const resolveRestaurantBySlug = (slug = "") =>
  require("./tenant-resolution.cjs").resolveRestaurantBySlug(slug);

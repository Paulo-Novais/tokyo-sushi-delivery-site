...
const normalizeLegacyDomainUser = (user = {}) => {
  ...
  const credentialStatus = status === "INACTIVE" ? "DISABLED" : status;
  const principalStatus = status === "INACTIVE" ? "BLOCKED" : status;
  const membershipStatus = status === "INACTIVE" ? "DISABLED" : status;
  const role = String(

...
const getRateLimitProfile = (options = {}) => {
  if (options.rateLimitProfile && RATE_LIMIT_PROFILES[options.rateLimitProfile]) {
    return RATE_LIMIT_PROFILES[options.rateLimitProfile];
  }

  if (options.routeType === "admin-auth" && options.action === "login") {
    return RATE_LIMIT_PROFILES.adminLogin;
  }

  if (options.routeType === "customer-auth") {
    return RATE_LIMIT_PROFILES.customerAuth;
  }

  if (options.routeType === "public-write") {
    return RATE_LIMIT_PROFILES.publicWrite;
  }

  if (options.routeType === "admin") {
    return RATE_LIMIT_PROFILES.adminApi;
  }

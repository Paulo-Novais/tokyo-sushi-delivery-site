...
-- Database-enforced read-only Support VIEW policies.
...
CREATE POLICY inovas_membership_select ON restaurant_memberships
FOR SELECT
USING (
  current_setting('app.audience', true) = 'system'
  OR (
...
CREATE POLICY inovas_membership_insert ON restaurant_memberships
FOR INSERT
WITH CHECK (
  current_setting('app.audience', true) = 'system'
  OR (
...
CREATE POLICY inovas_membership_update ON restaurant_memberships
FOR UPDATE
USING (
  current_setting('app.audience', true) = 'system'
  OR (
...
CREATE POLICY inovas_membership_delete ON restaurant_memberships
FOR DELETE
USING (
  current_setting('app.audience', true) = 'system'
  OR (
...
CREATE POLICY inovas_restaurant_binding_select ON restaurant_role_bindings
FOR SELECT
USING (
  EXISTS (
    SELECT 1
...
CREATE POLICY inovas_restaurant_binding_insert ON restaurant_role_bindings
FOR INSERT
WITH CHECK (
  EXISTS (
    SELECT 1
  ...
  AND (
    current_setting('app.audience', true) IN (
      'system', 'restaurant', 'provisioning'
    )
    OR (
      current_setting('app.audience', true) = 'support'
      AND current_setting('app.support_mode', true) = 'ADMIN'
...
CREATE POLICY inovas_restaurant_binding_update ON restaurant_role_bindings
FOR UPDATE
USING (
  EXISTS (
    SELECT 1
...
CREATE POLICY inovas_restaurant_binding_delete ON restaurant_role_bindings
FOR DELETE
USING (
  EXISTS (
    SELECT 1
  ...
  AND (
    current_setting('app.audience', true) IN ('system', 'restaurant')
    OR (
      current_setting('app.audience', true) = 'support'
      AND current_setting('app.support_mode', true) = 'ADMIN'
    )
...
CREATE POLICY inovas_user_audit_select ON user_audit_events
FOR SELECT
USING (
  current_setting('app.audience', true) = 'system'
  OR (
...
CREATE POLICY inovas_user_audit_insert ON user_audit_events
FOR INSERT
WITH CHECK (
  current_setting('app.audience', true) = 'system'
  OR (
